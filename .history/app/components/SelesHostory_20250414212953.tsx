    ...
    <div className="mt-6 bg-white p-6 rounded-lg shadow-md">
      ...
      {loading ? (
        ...
        <>
          ...
          <table className="min-w-full border-collapse border">
            <thead>
              <tr className="bg-white">
                <th className="border p-2 text-black text-right">المنتج</th>
                <th className="border p-2 text-black text-right">الكمية</th>
                <th className="border p-2 text-black text-right">السعر الكلي</th>
          ...
          {/* إجمالي المبيعات حسب الشهر والسنة */}
          <div className="mt-6">
            <h2 className="text-xl font-bold text-blue-600">إجمالي المبيعات</h2>
            ...
            <div className="mt-4">
              ...
              <ul className="list-disc ml-4 text-black">
                {Object.entries(totalByMonth).map(([month, total]) => (
                  <li key={month}>
                    <strong>{month}</strong> : {total.toFixed(2)} Kz
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="mt-4">
              ...
              <ul className="list-disc ml-4 text-black">
                {Object.entries(totalByYear).map(([year, total]) => (
                  <li key={year}>
                    <strong>{year}</strong> : {total.toFixed(2)} Kz
                  </li>
                ))}
              </ul>
            </div>
