    ...
    <div className="mt-6 bg-white p-6 rounded-lg shadow-md">
      ...
      {loading ? (
        ...
        <>
          ...
          <table className="min-w-full border-collapse border">
            <thead>
              <tr className="bg-white">
                ...
                <th className="border p-2 text-black">Gain</th>
                ...
                currentSales.map((sale) => (
                  <tr key={sale.id} className="border">
                    ...
                    <td className="border p-2 text-black">
                      {(sale.totalPrice - sale.purchasePrice).toFixed(2)} MRU
                    </td>
                    <td className="border p-2 text-black">
          ...
          {/* Totaux des ventes et des gains */}
          <div className="mt-6">
            <h2 className="text-xl font-bold text-blue-600">Totaux des ventes et des gains</h2>
            ...
            <div className="mt-4">
              ...
              <ul className="list-disc ml-4 text-black">
                {Object.entries(totalByMonth).map(([month, total]) => (
                  <li key={month}>
                    <strong>{month}</strong> : {total.toFixed(2)} MRU (Gain: {gainByMonth[month].toFixed(2)} MRU)
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="mt-4">
              ...
              <ul className="list-disc ml-4 text-black">
                {Object.entries(totalByYear).map(([year, total]) => (
                  <li key={year}>
                    <strong>{year}</strong> : {total.toFixed(2)} MRU (Gain: {gainByYear[year].toFixed(2)} MRU)
                  </li>
                ))}
              </ul>
            </div>
